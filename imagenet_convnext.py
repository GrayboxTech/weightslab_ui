...
class ConvNeXtBlock(nn.Module):
    def __init__(self, dim, drop=0.0):
        super().__init__()
        self.dwconv = Conv2dWithNeuronOps(dim, dim, kernel_size=7, padding=3, groups=dim)
...
class ConvNeXt(NetworkWithOps, nn.Module):
    def __init__(self, num_classes=200, drop=0.2):
        ...
        self.block1_1 = ConvNeXtBlock(dims[0], drop=0.0)
        ...
        self.block2_1 = ConvNeXtBlock(dims[1], drop=0.0)
        ...
        self.block4_1 = ConvNeXtBlock(dims[3], drop=drop)

        self.gap = nn.AdaptiveAvgPool2d(1)
        self.head_linear = LinearWithNeuronOps(dims[3], 256)
        self.head_act = nn.ReLU()
        self.head_dropout = nn.Dropout(drop)
        self.head_out = LinearWithNeuronOps(256, num_classes)
    ...
    def children(self):
        return [
            ...
            self.block1_1.dwconv, self.block1_1.bn, self.block1_1.pwconv1, self.block1_1.bn1, self.block1_1.pwconv2, self.block1_1.bn2,
            ...
            self.block2_1.dwconv, self.block2_1.bn, self.block2_1.pwconv1, self.block2_1.bn1, self.block2_1.pwconv2, self.block2_1.bn2,
            ...
            self.block4_1.dwconv, self.block4_1.bn, self.block4_1.pwconv1, self.block4_1.bn1, self.block4_1.pwconv2, self.block4_1.bn2,
    ...
    def define_deps(self):
        ...
        # Stage 1
        for block in [self.block1_0, self.block1_1]:
            deps.extend([
        ...
        # Stage 2
        for block in [self.block2_0, self.block2_1]:
            deps.extend([
                (prev, block.dwconv, DepType.INCOMING),
                (block.dwconv, block.bn, DepType.SAME),
                (block.bn, block.pwconv1, DepType.INCOMING),
        ...
        # Stage 3
        for block in [self.block3_0, self.block3_1]:
            deps.extend([
                (prev, block.dwconv, DepType.INCOMING),
                (block.dwconv, block.bn, DepType.SAME),
                (block.bn, block.pwconv1, DepType.INCOMING),
        ...
        # Stage 4
        for block in [self.block4_0, self.block4_1]:
            deps.extend([
                (prev, block.dwconv, DepType.INCOMING),
                (block.dwconv, block.bn, DepType.SAME),
                (block.bn, block.pwconv1, DepType.INCOMING),
...
def get_exp():
    ...
    exp = Experiment(
        ...
        name="v0",
        root_log_dir="tinyimagenet-exp-convnext",
        logger=Dash("tinyimagenet-exp-convnext"),
        skip_loading=False
    )
    return exp
